...
router.get("/:from/:to", async (req, res, next) => {
    ...
    var pathsArray = [];
    const result = await axios.get(`https://metro-route-api.herokuapp.com/route/${from}/${to}`)
    ...
    if (result) {
        ...
        // var j = 1;
        if (result != undefined && result.data != undefined && result.data.path != undefined && result.data.path.length != undefined)
